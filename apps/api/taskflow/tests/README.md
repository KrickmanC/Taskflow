...
This directory contains tests for the Taskflow application. The tests are organized using pytest.
...
Taskflow has two types of API endpoints:

1. **External API** (`taskflow.api`):
...
```bash
# Run unit tests
python -m pytest taskflow/tests/unit/

# Run API contract tests
python -m pytest taskflow/tests/contract/api/

# Run App contract tests
python -m pytest taskflow/tests/contract/app/

# Run smoke tests
python -m pytest taskflow/tests/smoke/
```
...
- `taskflow_server`: Live Django test server for HTTP-based smoke tests
...
2. Use the correct client fixture based on the API being tested:
   ...
   - For smoke tests with real HTTP, use `taskflow_server`
...
```bash
python -m pytest --cov=taskflow --cov-report=term --cov-report=html
```
