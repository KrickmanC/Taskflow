...
This guide explains how to write tests for Taskflow using our pytest-based testing strategy.
...
```python
import pytest
from taskflow.api.serializers import MySerializer
...
Smoke tests should be placed in `tests/smoke/` directory and use the `taskflow_server` fixture to test against a real HTTP server.
...
class TestCriticalFlow:
    @pytest.mark.django_db
    def test_login_flow(self, taskflow_server, create_user, user_data):
        # Get login URL
        url = f"{taskflow_server.url}/api/auth/signin/"
...
```python
from taskflow.tests.factories import UserFactory, WorkspaceFactory
...
python -m pytest -m unit --cov=taskflow
```
