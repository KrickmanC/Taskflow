...
from taskflow.db.models import User, Workspace, WorkspaceMember
...
def session_client(api_client, create_user):
    """Return a session authenticated API client for app API testing, which is what taskflow.app uses"""
...
def create_bot_user(db):
    ...
    user = User.objects.create(
        email=f"bot-{unique_id}@taskflow.so",
...
@pytest.fixture
def taskflow_server(live_server):
    """
