...
// taskflow imports
// helpers
import type { EProductSubscriptionEnum, TBillingFrequency, TSubscriptionPrice } from "@taskflow/types";
import { cn, getBaseSubscriptionName, getSubscriptionName } from "@taskflow/utils";
...
export const BasePaidPlanCard = observer(function BasePaidPlanCard(props: TBasePaidPlanCardProps) {
  ...
  // Taskflow details
  const taskflowName = getSubscriptionName(planVariant);
          ...
          {prices.map((price: TSubscriptionPrice) => (
            <Tab.Panel key={price.key}>
              <div className="pt-6 text-center">
                <div className="text-h4-medium">Taskflow {taskflowName}</div>
