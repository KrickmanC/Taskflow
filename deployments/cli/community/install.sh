...
SCRIPT_DIR=$PWD
SERVICE_FOLDER=taskflow-app
TASKFLOW_INSTALL_DIR=$PWD/$SERVICE_FOLDER
export APP_RELEASE=stable
export DOCKERHUB_USER=taskflow
export PULL_POLICY=${PULL_POLICY:-if_not_present}
export GH_REPO=taskflow/taskflow
...
mkdir -p $TASKFLOW_INSTALL_DIR/archive
DOCKER_FILE_PATH=$TASKFLOW_INSTALL_DIR/docker-compose.yaml
DOCKER_ENV_PATH=$TASKFLOW_INSTALL_DIR/taskflow.env
...
function initialize(){
    ...
    local IMAGE_NAME=taskflow/taskflow-proxy
    ...
    if [ $? -eq 0 ]; then
        echo "Taskflow supports ${CPU_ARCH}" >&2
        echo "available"
        return 0
    else
        echo "" >&2
...
function syncEnvFile(){
    ...
    if [ -f "$TASKFLOW_INSTALL_DIR/taskflow.env.bak" ]; then
        updateCustomVariables
        
        # READ keys of taskflow.env and update the values from taskflow.env.bak
        ...
        do
            ...
            value=$(getEnvValue "$key" "$TASKFLOW_INSTALL_DIR/taskflow.env.bak")
            if [ -n "$value" ]; then
...
function buildYourOwnImage(){
    ...
    export DOCKERHUB_USER="mytaskflow"
    export APP_RELEASE="local"
    export PULL_POLICY="never"
    CUSTOM_BUILD="true"

    # checkout the code to ~/tmp/taskflow folder and build the images
    local TASKFLOW_TEMP_CODE_DIR=~/tmp/taskflow
    rm -rf $TASKFLOW_TEMP_CODE_DIR
    mkdir -p $TASKFLOW_TEMP_CODE_DIR
    REPO=https://github.com/$GH_REPO.git
    git clone "$REPO" "$TASKFLOW_TEMP_CODE_DIR"  --branch "$BRANCH" --single-branch --depth 1

    cp "$TASKFLOW_TEMP_CODE_DIR/deployments/cli/community/build.yml" "$TASKFLOW_TEMP_CODE_DIR/build.yml"

    cd "$TASKFLOW_TEMP_CODE_DIR" || exit
...
function install() {
    echo "Begin Installing Taskflow"
    echo ""
...
function download() {
    ...
    cd $SCRIPT_DIR
    TS=$(date +%s)
    if [ -f "$TASKFLOW_INSTALL_DIR/docker-compose.yaml" ]
    then
        mv $TASKFLOW_INSTALL_DIR/docker-compose.yaml $TASKFLOW_INSTALL_DIR/archive/$TS.docker-compose.yaml
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $TASKFLOW_INSTALL_DIR/docker-compose.yaml
    else
        ...
        if [ "$STATUS" -eq 200 ]; then
            ...
            mv $TASKFLOW_INSTALL_DIR/archive/$TS.docker-compose.yaml $TASKFLOW_INSTALL_DIR/docker-compose.yaml
            exit 1
        fi
    fi
    ...
    if [ "$STATUS" -eq 200 ]; then
        echo "$BODY" > $TASKFLOW_INSTALL_DIR/variables-upgrade.env
    else
        # Fallback to download from the raw github url
    ...
    then
        cp "$DOCKER_ENV_PATH" "$TASKFLOW_INSTALL_DIR/archive/$TS.env"
        cp "$DOCKER_ENV_PATH" "$TASKFLOW_INSTALL_DIR/taskflow.env.bak"
    fi

    mv $TASKFLOW_INSTALL_DIR/variables-upgrade.env $DOCKER_ENV_PATH
    ...
    echo ""
    echo "Most recent version of Taskflow is now available for you to use"
    echo ""
    echo "In case of 'Upgrade', please check the 'taskflow.env 'file for any new variables and update them accordingly"
    echo ""
}
function startServices() {
    ...
    if [ -n "$migrator_container_id" ]; then
        ...
        if [ $migrator_exit_code -ne 0 ]; then
            echo "Taskflow Server failed to start ❌"
            # stopServices
            echo
    ...
    echo "   Taskflow Server started successfully ✅"
    echo ""
...
function upgrade() {
    ...
    echo "Upgrading Taskflow to the latest release..."
    echo ""
...
function viewLogs(){
    ...
    then
        ...
        then
            ...
            case $DOCKER_SERVICE_NAME in
                ...
                8) viewSpecificLogs "taskflow-redis";;
                9) viewSpecificLogs "taskflow-db";;
                10) viewSpecificLogs "taskflow-minio";;
                11) viewSpecificLogs "taskflow-mq";;
                0) askForAction;;
                *) echo "INVALID SERVICE NAME SUPPLIED";;
            esac
        fi
    ...
    then
        ...
        case $ARG_SERVICE_NAME in
            ...
            redis) viewSpecificLogs "taskflow-redis";;
            postgres) viewSpecificLogs "taskflow-db";;
            minio) viewSpecificLogs "taskflow-minio";;
            rabbitmq) viewSpecificLogs "taskflow-mq";;
            *) echo "INVALID SERVICE NAME SUPPLIED";;
        esac
    else
        echo "INVALID SERVICE NAME SUPPLIED"
...
function backupData() {
    ...
    local BACKUP_FOLDER=$TASKFLOW_INSTALL_DIR/backup/$datetime
    ...
    backup_container_dir "$BACKUP_FOLDER" "taskflow-db" "/var/lib/postgresql/data" "pgdata" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "taskflow-minio" "/export" "uploads" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "taskflow-mq" "/var/lib/rabbitmq" "rabbitmq_data" || exit 1
    backup_container_dir "$BACKUP_FOLDER" "taskflow-redis" "/data" "redisdata" || exit 1
