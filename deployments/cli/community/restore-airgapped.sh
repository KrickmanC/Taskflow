...
function restoreData() {
    ...
    AIRGAPPED_INSTALL_PATH="$HOME/taskflowairgapped"
    ...
    if command -v jq &> /dev/null; then
        dockerServiceStatus=$($COMPOSE_CMD ls --filter name=taskflow-airgapped --format=json | jq -r .[0].Status)
    else
        dockerServiceStatus=$($COMPOSE_CMD ls --filter name=taskflow-airgapped | grep -o "running" | head -n 1)
    fi

    if [[ $dockerServiceStatus == "running" ]]; then
        echo "Taskflow Airgapped is running. Please STOP the Taskflow Airgapped before restoring data."
        exit 1
    fi
