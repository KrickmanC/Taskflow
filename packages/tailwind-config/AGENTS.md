...
- Surfaces are **siblings**, not nested (in the same taskflow)
- Each surface should use its corresponding layer for nested elements
- Surfaces provide the base for stacking layers

**Exception - Different Taskflows**:

- Modals, overlays, and popovers exist on a **different taskflow** (different z-index/stacking context)
...
// ❌ Wrong: Surface nested in surface (same taskflow)
<div className="bg-surface-1">
  <div className="bg-surface-2">
...
// ✅ Correct: Modal on different taskflow
<div className="bg-canvas">
  {/* Main page content */}
  <div className="bg-surface-1">
    Page content
  ...
  {/* Modal overlay - different taskflow */}
...
```tsx
// ✅ Correct: Modal structure (different taskflow exception)
function PageWithModal() {
  return (
    <div className="bg-surface-1">
      ...
      {/* Modal - different taskflow, can use surface even with surface below */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50">
          <div className="bg-backdrop fixed inset-0" />
          <div className="bg-surface-1 rounded-lg shadow-lg p-6">
...
OR
Is this a modal/overlay on a different taskflow (z-index)?
...
### ❌ Mistake 2: Nested Surfaces (Same Taskflow)

```tsx
// ❌ Wrong: Nested surfaces in same taskflow
<div className="bg-surface-1">
  <div className="bg-surface-2">
    Nested surface
  </div>
...
// ✅ Correct: Exception - Modal on different taskflow
<div className="bg-canvas">
  <div className="bg-surface-1">Page content</div>
  <div className="fixed inset-0 z-50">
    <div className="bg-surface-1">Modal (different taskflow)</div>
  </div>
</div>
```
...
2. **Use Surfaces for Pages and Top-Level Containers**: Surfaces are siblings, not nested (except modals/overlays on different taskflows)
...
- [ ] Surfaces are siblings, not nested (except modals/overlays on different taskflows)
